interface IServiceOfRoom {
  ...
  const serviceOfRoom: IServiceOfRoom = {
    async getServices() {
      try {
        const response = await fetch("https://localhost:7132/getServices", {
          headers: {
    ...
    async getServiceByRoomId(roomId) {
        ...
        try {
          const response = await fetch(
            `https://localhost:7132/api/RoomService/room/${roomId}/services`,
            {
              method: "GET",
              headers: {
      ...
      async deleteRoomService(roomService) {
        ...
        try {
          const response = await fetch(
            `https://localhost:7132/deleteRoomService`,
            {
              method: "DELETE",
              headers: {
                Accept: "application/json, text/plain, */*",
      ...
      async createRoomService(roomService) {
        try {
          const response = await fetch(
            `https://localhost:7132/createRoomService`,
            {
              method: "POST",
              headers: {
                Accept: "application/json, text/plain, */*",
