...
function TourBookingDetail(props: IProps) {
  ...
  const [orderTourDetail, setOrderTourDetail] = useState<IOrderTourDetail | null>(null);
  ...
  useEffect(() => {
    const fetchTourBookingDetail = async () => {
      if (orderTourHeaderId !== null) {
        try {
          const tourBookingDetail = await orderTourDetailService.getOrderTourDetailByOrderTourHeaderId(orderTourHeaderId);
  ...
  return (
    <Modal className="pt-16" show={showTourBookingDetail} onHide={handleCloseModal} backdrop="static" keyboard={false} size="xl">
      <Modal.Header closeButton>
        <Modal.Title style={{ fontWeight: "bold" }}>{orderTourDetail?.tourName}</Modal.Title>
      </Modal.Header>
        ...
        <Row>
          ...
          <Col md={4}>
            <div>
              ...
              <p><strong>Room name:</strong> <span>{orderTourDetail?.tourOrderQuantity}</span></p>
              <p><strong>User name:</strong> <span>{orderTourHeader?.fullName}</span></p>
              <p><strong>User email:</strong> <span>{orderTourHeader?.email}</span></p>
              <p><strong>User phone:</strong> <span>{orderTourHeader?.phone}</span></p>
            </div>

            <div className="price-section">
              <h4 style={{ fontWeight: "bold", color: "rgb(255, 94, 31)" }}>Total Price: {orderTourHeader?.totalPrice} $</h4>
